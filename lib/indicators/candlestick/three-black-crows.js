export function threeblackcrows(input) {
    const { candles } = input;
    ...
    for (let i = 2; i < candles.length; i++) {
        ...
        const closesLower = candle2.close < candle1.close && candle3.close < candle2.close;
        // Check if opens are within previous candle's body
        const opensWithinBody = candle2.open <= candle1.open && candle2.open >= candle1.close &&
        ...
        const largeBodies = range1 > 0 && range2 > 0 && range3 > 0 &&
            ...
            body2 >= range2 * 0.6 &&
            body3 >= range3 * 0.6;
        const isThreeBlackCrows = isBearish1 && isBearish2 && isBearish3 &&
            closesLower && opensWithinBody && largeBodies;
        result.push(isThreeBlackCrows);
    }
    return result;
}
export class ThreeBlackCrowsPattern {
    constructor(input) {
        this.candles = [];
    ...
    nextValue(candle) {
        ...
        const result = threeblackcrows({ candles: this.candles });
        return result[result.length - 1] || false;
    }
    getResult() {
        return threeblackcrows({ candles: this.candles });
    }
}
ThreeBlackCrowsPattern.calculate = threeblackcrows;
