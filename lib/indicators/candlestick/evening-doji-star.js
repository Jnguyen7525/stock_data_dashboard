export function eveningdojistar(input) {
    const { candles } = input;
    if (!candles || candles.length < 3) {
        return new Array(candles?.length || 0).fill(false);
    }
    const result = [];
    // First two candles can't be an evening doji star
    ...
    for (let i = 2; i < candles.length; i++) {
        ...
        // 2. Second candle is a doji that gaps up
        ...
        const isEveningDojiStar = firstIsBullish &&
            firstHasLargeBody &&
            secondIsDoji &&
            gapsUp &&
            thirdIsBearish &&
...
export class EveningDojiStar {
    ...
    nextValue(candle) {
        ...
        const result = eveningdojistar({ candles: this.candles });
        return result[result.length - 1] || false;
    }
    getResult() {
        return eveningdojistar({ candles: this.candles });
    }
}
EveningDojiStar.calculate = eveningdojistar;
