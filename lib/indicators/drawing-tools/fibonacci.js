...
export function fibonacciExtensions(input) {
    const { high, low, trend = 'down' } = input;
    if (high <= low) {
        throw new Error('High must be greater than low');
    }
...
export function fibonacciProjection(input) {
...
Fibonacci.extensions = fibonacciExtensions;
Fibonacci.projection = fibonacciProjection;
